"""
Deep 2.5D U-Net (3 downsamples -> near-global receptive field) over K-slice volume context.
Best single architecture on honest group CV (~0.083 single seed +3D; deeper receptive field
localises targets better than the 2-downsample net; self-attention overfits and is omitted).
Elastic-only augmentation. Emits ensemble-format OOF + test proba, seed-ensembled.
"""
import os, sys, time
import numpy as np
...
class DeepUNet(nn.Module):
    def __init__(self, vocab, K, emb=16, base=48, p=0.30):
        ...
        cin = K * emb + 1
        self.e0 = _blk(cin, base); self.e1 = _blk(base, base * 2)
        self.e2 = _blk(base * 2, base * 4); self.e3 = _blk(base * 4, base * 8)
...
def _train(idxA, Tidx, tri, vocab, K, cfg, seed, bg):
    ...
    net = DeepUNet(vocab, K, base=cfg["base"], p=cfg["dropout"]).to(DEV)
...
DEFAULT = dict(base=48, dropout=0.30, lr=2e-3, wd=5e-4, epochs=160, bs=128, bg_w=0.10,
               elastic_a=3.0, res=4)


def run(K=11, cfg=None, seeds_oof=(0, 1), seeds_test=(0, 1, 2), save=True, name="deepnet"):
    cfg = {**DEFAULT, **(cfg or {})}
    ...
    N = len(idx_tr); oof = np.zeros((N, 18, 32, 32), np.float32)
    t0 = time.time()
    if seeds_oof:
    ...
    for sd in seeds_test:
        net = _train(idx_tr, Tidx, np.arange(N), vocab, K, cfg, seed=2000 + sd, bg=bg)
        test += _predict(net, idx_te, np.arange(len(idx_te)), K, bg)
    test /= len(seeds_test)
    if save:
        from ensemble import save_preds
        save_preds(name, oof, test)
    print(f"deepnet K={K} base={cfg['base']} done ({time.time()-t0:.0f}s)", flush=True)
    return oof, test
...
if __name__ == "__main__":
    ...
    print("deepnet group-CV:", round(D.tune_decision(oof, tr["V"], tr["T"])[0], 4))
